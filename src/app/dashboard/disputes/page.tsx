            ...
            {disputesWithProjects.length === 0 ? (
                <Card className="">
                    <div className="p-12 text-center text-muted-foreground">
                        ...
                        <p className="text-sm">You don't have any active disputes at the moment.</p>
                    </div>
                </Card>
            ) : (
                                    ...
                                    <div className="flex items-start justify-between">
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                <Badge variant="outline">{dispute.project?.title || "Unknown Project"}</Badge>
                                            ...
                                            <h3 className="text-xl font-semibold">{dispute.project?.title || "Dispute"}</h3>
                                            <p className="text-muted-foreground flex items-center gap-2 mt-1">
                                                <AlertTriangle className="h-4 w-4 text-amber-500" />
                                                Opened by: {dispute.opened_by.substring(0, 6)}...{dispute.opened_by.substring(38)}
                                            </p>
                                        </div>
                                    ...
                                    {dispute.status === "voting" ? (
                                        <>
                                            <div className="text-center mb-2">
                                                ...
                                                <div className="font-mono text-xl font-bold text-accent">Voting</div>
                                            </div>
                                        ...
                                        <div className="text-center py-4 bg-muted/20 rounded-lg">
                                            ...
                                            <Link href={`/dashboard/projects/${dispute.project_id}`}>
                                                <Button variant="ghost" className="w-full mt-2">View Project</Button>
                                            </Link>
                                        </div>
                                    )}
                                </div>
